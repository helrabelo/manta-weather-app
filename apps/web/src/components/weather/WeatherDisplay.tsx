...
export function WeatherDisplay({ weather, cityName, isDark = false }: WeatherDisplayProps) {
  ...
  return (
    ...
    >
      ...
      <p
        className={`animate-weather-enter text-[clamp(3.5rem,14vw,10rem)] font-light leading-none tracking-tight drop-shadow-sm ${primary}`}
      ...
      {weather.hourly && weather.hourly.length > 0 && (
        <div className="animate-weather-enter w-full max-w-lg mt-4 sm:mt-6" style={{ animationDelay: '400ms' }}>
          <HourlyForecast hours={weather.hourly} isDark={isDark} />
        </div>
      )}

      {weather.daily && weather.daily.length > 0 && (
        <div className="animate-weather-enter w-full max-w-lg mt-3 sm:mt-4" style={{ animationDelay: '500ms' }}>
